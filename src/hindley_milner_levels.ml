(* NOTE: See `https://okmij.org/ftp/ML/generalization.html`. *)
...
and levels =
  {
    mutable level_old : level;
    mutable level_new : level;
  }
  ...
  if t1 == t2 then (* t1 and t2 are physically the same *)
    ...
    | (TVar ({contents = Unbound (_, l1)} as tv1) as t1, (* unify two free vars *)
      ...
      if tv1 == tv2 then (* the same variable *)
        ()
      else if l1 > l2 then (* bind the higher-level var *)
        tv1 := Link t2
      else
        tv2 := Link t1
    ...
    | _ -> assert false (* everything else is the unification error *)
...
let force_delayed_adjustments () =
  ...
  and adjust_one acc =
    function
    | TArrow (_, _, ls) as ty when ls.level_old <= !current_level -> ty :: acc (* update later *)
    | TArrow (_, _, ls) when ls.level_old = ls.level_new -> acc (* already updated *)
    | TArrow (ty1, ty2, ls) ->
      (
        let level = ls.level_new in
        ls.level_new <- marked_level;
...
let () =
  ...
  let c1 = Lam("x", Lam("y", App (Var "x", Var "y"))) in
    ...
    TArrow
      (TArrow (TVar {contents = Unbound ("d", 1)},
       ...
       TArrow (TVar {contents = Unbound ("d", 1)},
               TVar {contents = Unbound ("e", 1)}, {level_old = 1; level_new = 1}),
       {level_old = 1; level_new = 1})
    = top_type_check (Let ("x", c1, Var "x")) in

  let
    TArrow (TVar {contents = Unbound ("b", 1)},
            TVar {contents = Unbound ("b", 1)}, {level_old = 1; level_new = 1})
    = top_type_check (Let ("y", Lam ("z", Var "z"), Var "y")) in
  ...
  let
    TArrow (TVar {contents = Unbound ("a", 1)},
            TArrow (TVar {contents = Unbound ("c", 1)},
                    TVar {contents = Unbound ("c", 1)}, {level_old = 1; level_new = 1}),
            {level_old = 1; level_new = 1})
    = top_type_check (Lam ("x", Let ("y", Lam ("z", Var "z"), Var "y"))) in

  let
    TArrow (TVar {contents = Link (TVar {contents = Unbound ("c", 1)})},
            TVar {contents = Link (TVar {contents = Unbound ("c", 1)})},
            {level_old = 1; level_new = 1})
    = top_type_check (Lam ("x", Let ("y", Lam ("z", Var "z"), App (Var "y", Var "x")))) in
  ...
  *)
    ...
    TVar
      {contents =
         Link
           (TArrow (TVar {contents = Unbound ("c", 1)},
                    TVar {contents = Unbound ("c", 1)}, {level_old = 1; level_new = 1}))}
    = top_type_check (Let ("id", id, App (Var "id", Var "id"))) in
  ...
  let
    ...
    = top_type_check (Let
                        ("x",
                         c1,
                         Let ("y", Let ("z", App(Var "x", id), Var "z"), Var "y"))) in
      ...
      (TVar
       ...
       TArrow
         (TVar
          ...
          {level_old = 1; level_new = 1}),
       {level_old = 1; level_new = 1})
    = top_type_check (Lam ("x", Lam("y",
                                    Let
                                      ("x",
                                       App (Var "x", Var "y"),
                                       Lam ("x", App (Var "y", Var "x")))))) in

  (* now sound generalization ! *)
  let
    TArrow (TVar {contents = Unbound ("a", 1)},
            TVar {contents = Unbound ("a", 1)}, {level_old = 1; level_new = 1})
    = top_type_check (Lam ("x", Let ("y", Var "x", Var "y"))) in
  ...
  let
    TArrow (TVar {contents = Unbound ("a", 1)},
            TArrow (TVar {contents = Unbound ("c", 1)},
                    TVar {contents = Unbound ("a", 1)}, {level_old = 1; level_new = 1}),
            {level_old = 1; level_new = 1})
    = top_type_check (Lam ("x", Let ("y", Lam ("z", Var "x"), Var "y"))) in
    ...
    TArrow
      (TVar
       ...
       TArrow (TVar {contents = Unbound ("b", 1)},
               TVar {contents = Unbound ("c", 1)}, {level_old = 1; level_new = 1}),
       {level_old = 1; level_new = 1})
    = top_type_check (Lam ("x", Let ("y", Lam ("z", App (Var "x", Var "z")), Var "y"))) in
  ...
  let
    TArrow
      (TVar
       ...
       {level_old = 1; level_new = 1})
    = top_type_check (Lam ("x", Lam("y", Let ("x", App (Var "x", Var "y"), App (Var "x", Var "y"))))) in
            ...
            Link
              (TArrow
                 (TArrow (TVar {contents = Unbound ("b", 1)},
                  ...
                  TVar {contents = Unbound ("c", 1)}, {level_old = 1; level_new = 1}))},
       TVar {contents = Unbound ("c", 1)}, {level_old = 1; level_new = 1})
    = top_type_check (Lam ("x", Let ("y", Let ("z", App(Var "x", id), Var "z"), Var "y"))) in
